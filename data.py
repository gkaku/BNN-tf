...
def __load_jpeg(filenames, channels=3):
    return tf.image.decode_jpeg(tf.read_file(filenames), channels), filenames

def __read_mnist(path, training=True):
    mnist = input_data.read_data_sets(path, reshape=False, validation_size=0)
    if training:
        return tf.identity(mnist.train.images, name="images"), tf.identity(mnist.train.labels, name="labels")
    else:
        return tf.identity(mnist.test.images, name="images"), tf.identity(mnist.test.labels, name="labels")
...
def get_data_provider(name, training=True):
    if name == 'imagenet':
        ...
        if training:
            ...
            return DataProvider(__read_cifar([os.path.join(data_dir, 'data_batch_5.bin')]),
                                [10000, 32, 32, 3], False)
    elif name == 'cifar100':
        ...
        return DataProvider(__read_mnist(data_dir, training), [60000, 28, 28, 1], training)
